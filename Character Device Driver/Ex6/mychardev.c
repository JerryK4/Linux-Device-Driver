...
struct mychardev_data {
    ...
    /* YÊU CẦU 1: Khai báo biến đếm số lần mở */
    int open_count; 
};
...
static int mychardev_open(struct inode *inode, struct file *file)
{
    ...
    /* Lock mutex de bao ve bien dem va qua trinh cap phat */
    if (mutex_lock_interruptible(&dev->lock))
        return -ERESTARTSYS;

    /* Xu ly Truncate (Xoa du lieu cu neu mode la Write Only) */
    if ((file->f_flags & O_ACCMODE) == O_WRONLY && (file->f_flags & O_TRUNC)) {
        mychardev_trim(dev);
    }

    /* YÊU CẦU 2 & 3: Tăng biến đếm và in log */
    dev->open_count++;
...
static int mychardev_release(struct inode *inode, struct file *file)
{
    ...
    /* Lock mutex khi thay doi bien dem */
    mutex_lock(&dev->lock);
    
    /* YÊU CẦU 2: Giảm biến đếm khi đóng */
    dev->open_count--;
...
static int __init mychardev_init(void)
{
    ...
    /* Check kernel version for class_create */
    ...
    pr_info("mychardev: registered scull model (major=%d, minor=%d)\n",
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("mychardev scull with open count");
