...
/* Tham số module giới hạn kích thước */
static int max_size = 1024 * 1024; // 1MB
...
static loff_t my_llseek(struct file *filp, loff_t off, int whence)
{
    ...
    switch(whence) {
        case SEEK_SET: /* 0: Vị trí tuyệt đối từ đầu file */
            new_pos = off;
            break;

        case SEEK_CUR: /* 1: Vị trí tương đối từ con trỏ hiện tại */
            new_pos = filp->f_pos + off;
            break;

        case SEEK_END: /* 2: Vị trí tương đối từ cuối file */
    ...
    pr_info("Mychardev Seek: Moved to position %lld\n", new_pos);
    return new_pos;
}
...
static ssize_t my_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    /* Kiểm tra giới hạn bộ nhớ (Bài 7) */
    if (*f_pos >= max_size) {
...
static struct file_operations my_fops = {
    ...
    .read = my_read,
    .write = my_write,
    .llseek = my_llseek, /* Đăng ký hàm llseek */
};
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("mychardev scull with llseek support");
